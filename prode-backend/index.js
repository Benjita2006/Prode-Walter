// prode-backend/index.js (VERSIÓN FINAL ARREGLADA)
...
const { 
    ...
    obtenerRanking // <--- ESTO YA ESTABA IMPORTADO, AHORA LO USAREMOS
} = require('./footballService'); 

const { obtenerPartidosDeAPI } = require('./apiFootballService');

// MODIFICACIÓN 1: Puerto dinámico para Railway
...
app.post('/api/admin/sync-matches', authenticateToken, async (req, res) => {
    ...
    if (!apiMatches || apiMatches.length === 0) {
        return res.status(500).json({ message: 'La API no devolvió partidos.' });
    }
    ...
    try {
        ...
        await conn.commit();
        conn.release();
        res.json({ success: true, message: `Sync Exitosa: ${creados} nuevos, ${actualizados} actualizados.` });
...
app.delete('/api/admin/matches', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        conn.release();
        res.json({ success: true, message: 'Se eliminaron todos los partidos y pronósticos.' });
    } catch (error) {
...
app.get('/api/admin/users', authenticateToken, async (req, res) => {
    ...
    if (result.success) res.status(200).json(result.users);
    else res.status(500).json({ message: 'Error interno al cargar usuarios.' });
});
...
// MODIFICACIÓN 2: AGREGAMOS LA RUTA DEL RANKING QUE FALTABA
...
app.get('/api/buscar-ligas', async (req, res) => {
    try {
        const API_KEY = process.env.API_FOOTBALL_KEY || 'TU_CLAVE_AQUI'; 
        const response = await axios.get('https://v3.football.api-sports.io/leagues', {
            params: { country: 'Argentina' },
            headers: { 'x-apisports-key': API_KEY }
        });
        // Lógica de filtrado...
        const resultados = response.data.response.map(item => item.league); // Simplificado para visualización
        res.json(resultados);
    } catch (error) {
