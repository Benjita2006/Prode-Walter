    ...
    function App() {
        // ESTADOS
        const [tickets, setTickets] = useState([]); // Lista de boletas del usuario
        const [ticketActivo, setTicketActivo] = useState(null); // La boleta seleccionada actualmente
        ...
        // --- CARGAR PRONÓSTICOS DE LA BOLETA ACTIVA ---
useEffect(() => {
    const cargarPronosticosDeBoleta = async () => {
        // Si no hay ticket seleccionado o no hay fecha abierta, reseteamos el estado temporal
        ...
        try {
            // Llamamos a tu API filtrando por el ID del Ticket
            const res = await fetch(`${API_URL}/api/predictions/my-predictions?ticketId=${ticketActivo.id}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();

            // Convertimos el array que viene del server en un objeto { matchId: resultado }
            const mapaPronosticos = {};
            data.forEach(p => {
                mapaPronosticos[p.match_id] = p.prediction;
            });

            // Actualizamos el estado temporal para que los MatchCard se marquen
    ...
    try {
        ...
        // Si el usuario tiene boletas, seleccionamos la primera por defecto
        if (data.length > 0) {
        ...
        // Llamar a fetchTickets cuando se abre una fecha para cargar las boletas de esa ronda
        useEffect(() => {
            if (fechaAbierta) {
                fetchTickets(fechaAbierta);
            } else {
                // Si se cierra la fecha, limpiamos las boletas mostradas
                setTickets([]);
                setTicketActivo(null);
            }
        }, [fechaAbierta, fetchTickets]);
                    ...
                    <>
                        {tickets.map(t => (
                            <button
                                ...
                                className={`btn-ticket-tab ${ticketActivo?.id === t.id ? 'active' : ''}`}
                            >
                                {t.ticket_name}
                            </button>
                        ))}
